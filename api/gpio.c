...
const uint16_t gpio_bases[NUMBER_GPIO] = {
  GPIO
};

vic_irq_t gpio_vic_irqs[NUMBER_GPIO] = {
  VIC_IRQ_GPIO
};
...
void gpio_handler_init(gpio_num_t gpio_no) {
  ...
  irq_handler_init(gpio_vic_irqs[gpio_get_index(gpio_no)], GPIO_NUM_IRQS, (callback_t *)callback[gpio_get_index(gpio_no)], gpio_bases[gpio_get_index(gpio_no)] + ADDR_GPIO_IRQ_VENABLE);
}
...
void gpio_enable_irq(gpio_num_t gpio_no, gpio_irq_t irq, bool enable) {
  ...
  if (enable) irq_enable (gpio_vic_irqs[gpio_get_index(gpio_no)], irq);
  else        irq_disable(gpio_vic_irqs[gpio_get_index(gpio_no)], irq);
}
