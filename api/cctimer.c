...
void cctimer_set_prescaler(cctimer_num_t dev, uint16_t prescaler) {
  ...
  if (prescaler > 255) prescaler = 255;  // 8 bit saturation
...
void cctimer_config(
    cctimer_num_t    dev,
    cctimer_subdev_t subdev,
    clk_sel_t        clk_sel,
    restart_sel_t    sta_sel,
    capture_sel_t    cap_sel,
    capcmp_mode_t    mode
  ) {
  ...
  switch (subdev) {
    case CCT_A : WRITE_REG_16(dev + ADDR_CCTIMER_CONFIG_A, cctimer_config.val); break;
    case CCT_B : WRITE_REG_16(dev + ADDR_CCTIMER_CONFIG_B, cctimer_config.val); break;
  }
}
...
void cctimer_set_compare(cctimer_num_t dev, cctimer_subdev_t subdev, uint16_t cmp_val) {
  ...
  switch (subdev) {
    case CCT_A : WRITE_REG_16(dev + ADDR_CCTIMER_CAPCMP_A, cmp_val); break;
    case CCT_B : WRITE_REG_16(dev + ADDR_CCTIMER_CAPCMP_B, cmp_val); break;
  }
}
...
uint16_t cctimer_get_compare(cctimer_num_t dev, cctimer_subdev_t subdev) {
  ...
  cmp = 0;
  switch (subdev) {
    case CCT_A : cmp = READ_REG_U16(dev + ADDR_CCTIMER_CAPCMP_A); break;
    case CCT_B : cmp = READ_REG_U16(dev + ADDR_CCTIMER_CAPCMP_B); break;
  }
  if (cmp) cmp++;
  return cmp;
}
...
uint16_t cctimer_get_counter(cctimer_num_t dev, cctimer_subdev_t subdev) {
  ...
  cnt = 0;
  switch (subdev) {
    case CCT_A : cnt = READ_REG_U16(dev + ADDR_CCTIMER_COUNTER_A); break;
    case CCT_B : cnt = READ_REG_U16(dev + ADDR_CCTIMER_COUNTER_B); break;
  }
  return cnt;
}
...
uint16_t cctimer_get_capture(cctimer_num_t dev, cctimer_subdev_t subdev) {
  ...
  cap = 0;
  switch (subdev) {
    case CCT_A : cap = READ_REG_U16(dev + ADDR_CCTIMER_CAPCMP_A); break;
    case CCT_B : cap = READ_REG_U16(dev + ADDR_CCTIMER_CAPCMP_B); break;
  }
  if (cap) cap++;
  return cap;
}
...
void cctimer_set_presel(
  ...
  switch (subdev) {
    case CCT_A : WRITE_REG_16(dev + ADDR_CCTIMER_PRESEL_A, cctimer_presel.val); break;
    case CCT_B : WRITE_REG_16(dev + ADDR_CCTIMER_PRESEL_B, cctimer_presel.val); break;
  }
}
...
const uint16_t cctimer_bases[NUMBER_CCTIMER] = {
  CCTIMER
};

vic_irq_t cctimer_vic_irqs[NUMBER_CCTIMER] = {
  VIC_IRQ_CCTIMER
};
...
uint16_t cctimer_get_index(cctimer_num_t cctimer_no) {
  ...
  for (n = 1; n < NUMBER_CCTIMER; n++) {
    if (cctimer_no == cctimer_bases[n]) return n;
  }
  return 0;
}
...
void cctimer_handler_init(cctimer_num_t cctimer_no) {
    ...
    irq_handler_init(cctimer_vic_irqs[cctimer_get_index(cctimer_no)], CCTIMER_NUM_IRQS, (callback_t *)callback[cctimer_get_index(cctimer_no)], cctimer_bases[cctimer_get_index(cctimer_no)] + ADDR_CCTIMER_IRQ_VENABLE);
}
...
void cctimer_enable_irq(cctimer_num_t cctimer_no, cctimer_irq_t irq, bool enable) {
    ...
    if (enable) irq_enable (cctimer_vic_irqs[cctimer_get_index(cctimer_no)], irq);
    else        irq_disable(cctimer_vic_irqs[cctimer_get_index(cctimer_no)], irq);
}
