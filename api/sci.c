...
void _sci_timer_prepare(uintptr_t dev, bool val) {
  ...
  SCI_LINMODE.val = SCI(dev)->lin_mode.val;
  SCI_LINMODE.bf.timer_prepare = val;
  SCI(dev)->lin_mode.val = SCI_LINMODE.val;
}
...
bool _sci_get_rxd(uintptr_t dev){
  ...
  SCI_LINMODE.val = SCI(dev)->lin_mode.val;
  if (SCI_LINMODE.bf.rxd) return true;
  else return false;
}
...
{
  return SCI(dev)->lin_configuration.val;
}

void _sci_set_data(uintptr_t dev,uint8_t byte)
{
   SCI(dev)->data_io = ((sci_data_io_t)byte);
}

uint8_t _sci_get_data(uintptr_t dev)
{
   return (uint8_t)((SCI(dev)->data_io)&MSK_SCI_DATA_IO);
}


void _sci_setup_tx_dma(uintptr_t dev,uint8_t *buf, uint16_t len)
{
  (SCI(dev)->dma_tx_length)=0;
  (SCI(dev)->dma_tx_address)=(uint16_t)(buf);
  (SCI(dev)->dma_tx_length)=len;  
}

void _sci_setup_rx_dma(uintptr_t dev,uint8_t *buf, uint16_t len)
{
  (SCI(dev)->dma_rx_length)=0;
  (SCI(dev)->dma_rx_address)=(uint16_t)(buf);
  (SCI(dev)->dma_rx_length)=len;  
}
...
{
  (SCI(dev)->timer_compare)=cmp;
}

uint16_t _sci_get_timer_counter(uintptr_t dev)
{
  return (SCI(dev)->timer_counter);  
}

uint16_t _sci_get_timer_compare(uintptr_t dev)
{
  return (SCI(dev)->timer_compare);
}

void _sci_enable_addon_irq(uintptr_t dev,uint16_t mask,bool en)
{
  if (en) (SCI(dev)->addon_interrupt_enable.val)|=mask;
  else (SCI(dev)->addon_interrupt_enable.val)&=~mask;
}

void _sci_reset_addon_irq(uintptr_t dev,uint16_t mask)
{
  (SCI(dev)->addon_interrupt_status.val)=mask;
}
...
const uint16_t sci_bases[NUMBER_SCI] = {
  SCI
};

vic_irq_t sci_vic_irqs[NUMBER_SCI] = {
  VIC_IRQ_SCI
};
...
uint16_t sci_get_index(sci_num_t sci_no) {
  ...
  if (sci_no == sci_bases[1]) return 1;
  return 0;
}
...
void sci_handler_init(sci_num_t sci_no) {
  ...
  irq_handler_init(sci_vic_irqs[sci_get_index(sci_no)], SCI_NUM_IRQS, (callback_t *)callback[sci_get_index(sci_no)], sci_bases[sci_get_index(sci_no)] + ADDR_SCI_IRQ_VENABLE);
}
...
void sci_enable_irq(sci_num_t sci_no, sci_irq_t irq, bool enable) {
  ...
  if (enable) irq_enable (sci_vic_irqs[sci_get_index(sci_no)], irq);
  else        irq_disable(sci_vic_irqs[sci_get_index(sci_no)], irq);
}
...
sci_irq_t _sci_get_pending_irq(uintptr_t dev)
{
  ...
  uint16_t irqs=(SCI(dev)->irq_status.val);
  int num=0;
