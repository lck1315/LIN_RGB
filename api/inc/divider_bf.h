...
typedef struct {
  volatile divider_op1_t                           op1lo;                          /* 0 */
  volatile divider_op1_t                           op1hi;                          /* 2 */
  volatile divider_op2_t                           op2;                            /* 4 */
  volatile divider_op2s_t                          op2s;                           /* 6 */
  volatile divider_result_t                        resultlo;                       /* 8 */
  volatile divider_result_t                        resulthi;                      /* 10 */
  volatile divider_remainder_t                     remainder;                     /* 12 */
  unsigned short                                   reserved0[17];
  volatile divider_irq_status_t                    irq_status;                    /* 48 */
  unsigned short                                   reserved1[1];
  volatile divider_irq_mask_t                      irq_mask;                      /* 52 */
  unsigned short                                   reserved2[1];
  volatile divider_irq_venable_t                   irq_venable;                   /* 56 */
  volatile divider_irq_vdisable_t                  irq_vdisable;                  /* 58 */
  volatile divider_irq_vmax_t                      irq_vmax;                      /* 60 */
  volatile divider_irq_vno_t                       irq_vno;                       /* 62 */
} _PACKED_ divider_t;
