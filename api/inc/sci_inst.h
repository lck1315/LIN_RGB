...
void _sci_timer_prepare(uintptr_t dev, bool val);
...
void _sci_setup_tx_dma(uintptr_t dev,uint8_t *buf, uint16_t len);
void _sci_setup_rx_dma(uintptr_t dev,uint8_t *buf, uint16_t len);
void _sci_stop_tx_dma(uintptr_t dev);
void _sci_stop_rx_dma(uintptr_t dev);
...
#define sci_set_timer_compare(...)       _sci_set_timer_compare(SCI, __VA_ARGS__)
#define sci_get_timer_counter()          _sci_get_timer_counter(SCI)
#define sci_get_timer_compare()          _sci_get_timer_compare(SCI)

#define sci_enable_addon_irq(...)        _sci_enable_addon_irq(SCI, __VA_ARGS__)
#define sci_reset_addon_irq(...)         _sci_reset_addon_irq(SCI, __VA_ARGS__)

/*---*/

#define trgt_sci_set_config(...)        _sci_set_config       (0x0980, __VA_ARGS__)
#define trgt_sci_reset_config(...)      _sci_reset_config     (0x0980, __VA_ARGS__)
#define trgt_sci_transmit(...)          _sci_transmit         (0x0980, __VA_ARGS__)
#define trgt_sci_receive()              _sci_receive          (0x0980)
#define trgt_sci_transmit_byte(...)     _sci_transmit_byte    (0x0980, __VA_ARGS__)
#define trgt_sci_receive_byte(...)      _sci_receive_byte     (0x0980, __VA_ARGS__)
#define trgt_sci_get_status()           _sci_get_status       (0x0980)
#define trgt_sci_set_baud_rate(...)     _sci_set_baud_rate    (0x0980, __VA_ARGS__)
#define trgt_sci_get_baud_rate()        _sci_get_baud_rate    (0x0980)
#define trgt_sci_set_auto_mode(...)     _sci_set_auto_mode    (0x0980, __VA_ARGS__)
#define trgt_sci_set_meas_mode(...)     _sci_set_meas_mode    (0x0980, __VA_ARGS__)
#define trgt_sci_get_meas_value()       _sci_get_meas_value   (0x0980)
#define trgt_sci_meas_enable()          _sci_meas_enable      (0x0980)
#define trgt_sci_meas_disable()         _sci_meas_disable     (0x0980)
#define trgt_sci_set_dbc_filter(...)    _sci_set_dbc_filter   (0x0980, __VA_ARGS__)
#define trgt_sci_get_dbc_filter()       _sci_get_dbc_filter   (0x0980)
#define trgt_sci_bus_collision_detect(...)  _sci_bus_collision_detect (0x0980, __VA_ARGS__);
#define trgt_sci_timer_enable(...)      _sci_timer_enable     (0x0980, __VA_ARGS__)
#define trgt_sci_timer_prepare(...)     _sci_timer_prepare    (0x0980, __VA_ARGS__)
#define trgt_sci_timeout_enable(...)    _sci_timeout_enable   (0x0980, __VA_ARGS__)
#define trgt_sci_disable(...)           _sci_disable          (0x0980, __VA_ARGS__)
#define trgt_sci_set_txd(...)           _sci_set_txd          (0x0980, __VA_ARGS__)
#define trgt_sci_get_rxd()              _sci_get_rxd          (0x0980)
